...
const inviteRegex = / invited you to \?[a-z0-9]{8}$/;
const HackChatSession = (function (_super) {
  __extends(HackChatSession, _super);
  function HackChatSession(channel, username, password, options) {
    if (options === void 0) {
      options = { server: "wss://hack.chat/chat-ws" };
    }
    _super.call(this);
          ...
          switch (String(json.cmd)) {
            ...
            case "info":
              ...
              if (inviteRegex.test(json.text))
                return this.emit(
                  "invitation",
                  String(json.text).substr(0, String(json.text).length - 25),
                  String(json.text).substr(-8),
                  json.time || 0
                );
              if (json.text.indexOf("You invited") == 0)
                return this.emit(
                  "invited",
                  String(json.text).substr(12, String(json.text).length - 25),
                  String(json.text).substr(-8),
                  json.time || 0
                );
...
const HackChat = (function (_super) {
  ...
  HackChat.prototype.join = function (channel, username, password, options) {
    ...
    const session = new HackChatSession(channel, username, password, options);
    session.on(
      "joining",
      function () {
    ...
    session.on(
      "chat",
      function (nick, text, time, isAdmin, trip) {
